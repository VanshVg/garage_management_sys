let generateInvoice = async (appointmentId, customerEmail, downloadBtn) => {
...
<p class='ml-1 font-bold'>Downloading...</p>
  ...
  if (response.success) {
    ...
    download.id = "download-" + appointmentId;
    download.setAttribute("href", `/invoices/${response.pdf}.pdf`);
    let date = new Date().toISOString().split("T")[0];
    download.setAttribute("download", `invoice-${date}.pdf`);
