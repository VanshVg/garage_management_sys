const editGarage = async (index) => {
  ...
  document.querySelector('#editGarage').setAttribute("garageId", garage.garage_id);
  ...
  document.querySelector('#editGarage #contactNumber').value = garage.contact_number;
  document.querySelector('#editGarage #openTime').value = garage.open_time.slice(11, 16);
  document.querySelector('#editGarage #closeTime').value = garage.close_time.slice(11, 16);
  document.querySelector('#editGarage #description').value = garage.description;
  document.querySelector('#editGarage  img').src = '/uploads/' + garage.thumbnail;
  document.querySelector('#editGarage  img').classList.remove('hidden');
  const addressDetails = await callAPI('/owner/garages/address/' + garage.garage_id);
