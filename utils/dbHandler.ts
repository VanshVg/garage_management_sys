...
export const findOneById = async (userId: string | number) => {
  try {
...
export const activateUser = async (userId: string | number) => {
  try {
...
// insert new slots
export const insertSlot = async (slotTime: Array<string | number>) => {
  try {
    let query = `INSERT INTO slot_master (garage_id,start_time,end_time,availability_status) values (?)`;
    let results: Array<ResultSetHeader> = (await conn.query(query, [
      slotTime,
...
export const updateSlot = async (slotTime: Array<string | number>) => {
  try {
...
export const deleteSlot = async (slotId: Array<string | number>) => {
  try {
...
export const updateUserByEmail = async (userInfo: Array<number | string>) => {
  try {
    ...
    let results: Array<ResultSetHeader> = (await conn.query(
      query,
      userInfo
    )) as Array<ResultSetHeader>;
...
export const insertAddress = async (userInfo: Array<string | number>) => {
  try {
...
export const insertUserAddress = async (userInfo: Array<string | number>) => {
  try {
...
export const updateAddressById = async (userInfo: Array<string | number>) => {
  try {
...
// delete user address
export const deleteUserAddress = async (userInfo: Array<string | number>) => {
  try {
...
export const insertGarageOwner = async (ownerInfo: Array<string | number>) => {
  try {
...
export const insertGarageAddress = async (
  addressInfo: Array<string | number>
) => {
  try {
    let query = `INSERT INTO address_master (city_id, area, pincode) values (?)`;
    let result: Array<ResultSetHeader> = (await conn.query(query, [
...
export const insertGarageReference = async (
...
// update garage
export const updateGarage = async (garageInfo: Array<string | number>) => {
  try {
    ...
    let result: Array<ResultSetHeader> = (await conn.query(
      query,
      garageInfo
    )) as Array<ResultSetHeader>;
    return result[0].affectedRows;
...
export const updateGarageAddress = async (
  addressInfo: Array<string | number>
) => {
  try {
...
export const getNotAvailableService = async (id: Array<string | number>) => {
  try {
    let query =
...
export const getSingleGarageService = async (garageId: string) => {
  try {
...
export const insertService = async (serviceInfo: Array<string>) => {
  try {
...
export const insertGarageService = async (
  serviceInfo: Array<string | number>
) => {
  try {
...
export const getOwnerGarages = async (ownerId: number | undefined) => {
  try {
...
export const deleteGarageService = async (
  serviceInfo: Array<string | number>
) => {
  try {
...
export const selectByFieldName = async (
  tableName: string,
  fieldName: string,
  value: unknown
) => {
...
export const serviceListing = async (garageId: number | string) => {
  try {
...
export const selectByFieldNames = async (tableName: string, fields: object) => {
  try {
    ...
    keys.forEach((element: string) => {
      if (i != keys.length - 1) {
        query += `${element} = "${
          fields[element as keyof typeof fields]
        }" AND `;
      } else {
...
export const countServices = async (ownerId: number | undefined) => {
  try {
    let query =
...
export const findVehicleData = async (email: string, type: string | number) => {
  try {
...
export const getAppointments = async (
  ownerDetails: Array<string | number | undefined>
) => {
  try {
...
export const customerSlotListing = async (
  garageId: string,
  date: string,
  date2: string
) => {
  console.log(garageId, date, date2);
  try {
...
export const garageSlotListing = async (
  garageId: number,
  startDate: string,
  endDate: string
) => {
...
export const getGarageAddress = async (garageId: Array<string>) => {
  try {
    const query =
...
export const insertFeedback = async (
  ...
  try {
    ...
    const result: Array<ResultSetHeader> = (await conn.query(query, [
      garageId,
      customerId,
      description,
      rating,
...
export const getInvoiceDetails = async (
  appointmentDetails: [string, number | undefined]
) => {
  try {
...
export const ifFeedbackExist = async (customerId: number) => {
  try {
...
export const getAppointsByDateRange = async (
...
export const updateFields = async (
  ...
  try {
    ...
    if (keys.length == 1) {
      query += `${keys[0]} = ${
        conditions[keys[0] as keyof typeof conditions]
      };`;
    } else {
      let i = 0;
      keys.forEach((element) => {
        if (i != keys.length - 1) {
          query += `${element} = "${
            conditions[element as keyof typeof conditions]
          }" AND `;
        } else {
          query += `${element} = "${
            conditions[element as keyof typeof conditions]
...
export const getGarageAppointments = async (
  garageId: string,
  limit: number | undefined
) => {
  try {
    let query = `
    SELECT users.name AS customer_name, users.email AS customer_email, slot_master.start_time, appointments.id AS appointment_id, appointment_payments.status AS payment_status, invoice_url, appointments.status AS appointment_status FROM appointments JOIN slot_master ON appointments.slot_id = slot_master.id JOIN appointment_payments ON appointment_payments.appointment_id = appointments.id JOIN users ON users.id = appointments.customer_id WHERE slot_master.garage_id=? AND (appointments.status = 2 OR appointments.status = 4 ) LIMIT ?, 10;
    SELECT COUNT(users.name) AS count FROM appointments JOIN slot_master ON appointments.slot_id = slot_master.id JOIN appointment_payments ON appointment_payments.appointment_id = appointments.id JOIN users ON users.id = appointments.customer_id WHERE slot_master.garage_id=? AND (appointments.status = 2 OR appointments.status = 4 );`;
...
export const countRevenue = async (userId: number | undefined) => {
  try {
...
export const handleUpdateAppointments = async (
...
export const findVehicleStatus = async (
  garageId: string,
  limit: number | undefined
) => {
  try {
...
// get user vehicle
export const fetchUserVehicle = async (id: string | number) => {
  try {
...
export const updateVehicleDetails = async (
  vehicleInfo: Array<string | number>
) => {
  try {
